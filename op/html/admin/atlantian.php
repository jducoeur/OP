...
else if (isset($_GET['atlantian_id']))
{
   $form_atlantian_id = clean($_GET['atlantian_id']);
}
...
if (isset($_SESSION[$OP_ADMIN]) && $_SESSION[$OP_ADMIN])
{
   ...
   if ($mode == $MODE_EDIT && $form_atlantian_id == 0)
   {
   ...
   if (isset($_POST['submit']))
   {
      if ($_POST['submit'] == $SUBMIT_DELETE)
      {
         if ($mode == $MODE_EDIT && $form_atlantian_id > 0)
         {
            ...
            $check_query = "SELECT atlantian_id FROM $DBNAME_OP.atlantian_award WHERE atlantian_id = $form_atlantian_id";
            $check_result = mysql_query($check_query)
               or die("Error checking OP for Atlantian: " . mysql_error());
            $count += mysql_num_rows($check_result);

            // Check for child order records: orders, users
            $check_query = "SELECT atlantian_id FROM $DBNAME_AUTH.user_auth WHERE atlantian_id = $form_atlantian_id";
            $check_result = mysql_query($check_query)
               or die("Error checking Unified for Atlantian: " . mysql_error());
            $count += mysql_num_rows($check_result);
            ...
            {
               // Delete Atlantian
               $delete = "DELETE FROM $DBNAME_AUTH.atlantian WHERE atlantian_id = $form_atlantian_id";
               $del_result = mysql_query($delete)
                  or die("Error deleting Atlantian: " . mysql_error());
            ...
            else
            {
               ...
               $errmsg .= " for Atlantian ID $form_atlantian_id; this Atlantian may not be deleted until all child records are deleted.";
            }
            // Close DBs
            db_disconnect($link);
         }
      ...
      if ($_POST['submit'] == $SUBMIT_SAVE)
      {
         ...
         if ($valid)
         {
            ...
            if ($form_atlantian_id > 0)
            {
               ...
               $name_query = "SELECT sca_name, preferred_sca_name FROM $DBNAME_AUTH.atlantian WHERE atlantian_id = " . value_or_null($form_atlantian_id);
            ...
            else
            {
               ...
               $sql_stmt = "INSERT INTO $DBNAME_AUTH.atlantian (first_name, middle_name, last_name, email, gender, deceased, deceased_date, inactive, revoked, revoked_date, " .
               ...
               $sql_result = mysql_query($sql_stmt)
                  or die("Error inserting Atlantian data: " . mysql_error());
   ...
   // Read Existing Atlantian - if this isn't a submit, or it is a failed submit delete
   if ($form_atlantian_id > 0 && (!(isset($_POST['submit'])) || (isset($_POST['submit']) && $_POST['submit'] == $SUBMIT_DELETE)))
   {
      $link = db_connect();
      $query = "SELECT * FROM $DBNAME_AUTH.atlantian WHERE atlantian_id = " . value_or_null($form_atlantian_id);
...
<table border="0" align="center" cellpadding="5" summary="">
   ...
   <tr><td colspan="2" align="center"><a href="../op_ind.php?atlantian_id=<?php echo $form_atlantian_id; ?>">View Individual Page</a></td></tr>
</table>
<?php
         $link = db_connect();
         $award_query = "SELECT atlantian_award.*, award.award_name, branch.branch, rg.branch AS rg, " .
                        ...
                        "FROM $DBNAME_OP.atlantian_award JOIN award ON atlantian_award.award_id = award.award_id " .
                        ...
                        "WHERE atlantian_id = $form_atlantian_id ORDER BY precedence, award_date, sequence";
