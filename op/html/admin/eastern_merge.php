...
$first_eastern_id = 0;
if (isset($_REQUEST['first_eastern_id']))
{
   $first_eastern_id = clean($_REQUEST['first_eastern_id']);
}

$second_eastern_id = 0;
if (isset($_REQUEST['second_eastern_id']))
{
   $second_eastern_id = clean($_REQUEST['second_eastern_id']);
}

if (isset($_SESSION[$OP_ADMIN]) && $_SESSION[$OP_ADMIN])
{
   if ($first_eastern_id == 0 || $second_eastern_id == 0)
   {
   ...
   if (isset($_POST['submit']) && $_POST['submit'] == $SUBMIT_MERGE)
   {
      ...
      if ($errmsg == '')
      {
         ...
         // GET SELECTED AWARDS
         $form_eastern_award_id = "";
         if (isset($_POST['form_eastern_award_id']))
         {
            $form_eastern_award_id = create_in_clause_from_array($_POST['form_eastern_award_id']);
         }
         ...
         if ($errmsg == '')
         {
            ...
            $keep_eastern_id = $first_eastern_id;
            $del_eastern_id = $second_eastern_id;

            $link = db_admin_connect();

            // Is one of these records tied to an account?  Make sure the first Easterner ID is listed first
            $sort_dir = "ASC";
            if ($first_eastern_id < $second_eastern_id)
            {
               $sort_dir = "DESC";
            }
            $account_query = "SELECT * FROM $DBNAME_AUTH.user_auth WHERE eastern_id IN (" . $first_eastern_id . ", " . $second_eastern_id . ") ORDER BY eastern_id $sort_dir";
            ...
            // If one account is tied, use that Easterner ID
            if ($num_accounts == 1)
            {
               $account_data = mysql_fetch_array($account_result, MYSQL_BOTH);
               $account_eastern_id = $account_data['eastern_id'];
               if ($account_eastern_id == $second_eastern_id)
               {
                  $keep_eastern_id = $second_eastern_id;
                  $del_eastern_id = $first_eastern_id;
               }
            }
            // If both Easterner IDs have accounts, keep the account most recently logged in
            else if ($num_accounts == 2)
            {
               ...
               $account_data = mysql_fetch_array($account_result, MYSQL_BOTH);
               $account_eastern_id2 = $account_data['eastern_id'];
               ...
               {
                  // Account with second Easterner ID was used most recently
               ...
               // Account with second Easterner ID is the only one that has been used
            ...
            // If not, use the lower Easterner ID
            else
            {
               if ($first_eastern_id > $second_eastern_id)
               {
            ...
            // Update the selected Easterner ID with all the selected data fields

            // Get previous SCA Name
            $name_query = "SELECT sca_name, preferred_sca_name FROM $DBNAME_AUTH.eastern WHERE eastern_id = " . value_or_null($keep_eastern_id);
            ...
            $sql_stmt = "UPDATE $DBNAME_AUTH.eastern SET " .
               ...
               " WHERE eastern_id = " . value_or_null($keep_eastern_id);
            ...
            // UPDATE OP DATA
            //------------------------
            $eastern_award_clause = "";
            if ($form_eastern_award_id != "")
            {
               $eastern_award_in_clause = " AND eastern_award_id IN (" . $form_eastern_award_id . ")";
               $eastern_award_not_in_clause = " AND eastern_award_id NOT IN (" . $form_eastern_award_id . ")";
            }

            // Update all selected awards with the selected Easterner ID
            $upd_award = "UPDATE $DBNAME_OP.eastern_award SET " .
               "eastern_id = " . value_or_null($keep_eastern_id) .
               ", last_updated = " . value_or_null(date("Y-m-d")) .
               ", last_updated_by = " . value_or_null($_SESSION['s_user_id']) . 
               " WHERE eastern_id = " . value_or_null($del_eastern_id) . 
               $eastern_award_in_clause;
            $del_result = mysql_query($upd_award)
               or die("Error updating Easterner awards in OP: " . mysql_error());

            // Update all non-selected awards with the to-be-deleted Easterner ID
            $upd_award = "UPDATE $DBNAME_OP.eastern_award SET eastern_id = $del_eastern_id WHERE eastern_id = $keep_eastern_id " . $eastern_award_not_in_clause;
            $del_result = mysql_query($upd_award)
               or die("Error updating Easterner awards in OP: " . mysql_error());

            // Delete all remaining awards with the non-selected Easterner ID
            $delete = "DELETE FROM $DBNAME_OP.eastern_award WHERE eastern_id = $del_eastern_id";
            $del_result = mysql_query($delete)
               or die("Error deleting Easterner awards from OP: " . mysql_error());

            //------------------------
            // UPDATE UNIVERSITY DATA
            //------------------------
            // How many Participant records with these two Easterner IDs?
            $query = "SELECT * FROM $DBNAME_UNIVERSITY.participant " . 
               "WHERE eastern_id IN (" . value_or_null($keep_eastern_id) . ", " . value_or_null($del_eastern_id) . ") " .
               "ORDER BY eastern_id $sort_dir";
            $result = mysql_query($query)
               or die("Participant Query Failed: " . $query . "<br/>" . mysql_error());
            $num_participants = mysql_num_rows($result);

            // Just one?  Update participant record with keep Easterner ID
            ...
            else if ($num_participants == 2)
            {
               ...
               // Update remaining participant record with keep Easterner ID
               $update = "UPDATE $DBNAME_UNIVERSITY.participant SET " .
                  "eastern_id = " . value_or_null($keep_eastern_id) .
            ...
            // UPDATE AUTH DATA
            //------------------------
            // Delete user account for non-selected Easterner ID
            if ($num_accounts == 2)
            {
               $delete = "DELETE FROM $DBNAME_AUTH.user_auth WHERE eastern_id = $del_eastern_id";
               $del_result = mysql_query($delete)
                  or die("Error deleting Easterner from user_auth: " . mysql_error());
               ...
               $update = "UPDATE $DBNAME_AUTH.user_auth $set_clause WHERE eastern_id = $keep_eastern_id";
               $update_result = mysql_query($update)
                  or die("Error updating permissions in user_auth: " . mysql_error());
            }

            // Delete non-selected Easterner ID from auth
            $delete = "DELETE FROM $DBNAME_AUTH.eastern WHERE eastern_id = $del_eastern_id";
            $del_result = mysql_query($delete)
               or die("Error deleting Easterner from auth: " . mysql_error());

            // Close DB
            db_disconnect($link);

            // Redirect to edit page
            redirect("eastern.php?eastern_id=$keep_eastern_id&mode=$MODE_EDIT");
         }
      } // Valid
   } // Submit

   //-------------------------------
   // Read Existing Easterner if this isn't a submit
   //-------------------------------
   if ($first_eastern_id > 0 && $second_eastern_id > 0 && ((!(isset($_POST['submit']))) || (isset($_POST['submit']) && $errmsg != '')))
   {
      $link = db_connect();
      // Collect data on first Eastern
      $query = "SELECT eastern.*, heraldic_rank.heraldic_rank FROM $DBNAME_AUTH.eastern LEFT OUTER JOIN $DBNAME_WARRANT.heraldic_rank ON eastern.heraldic_rank_id = heraldic_rank.heraldic_rank_id WHERE eastern.eastern_id = " . value_or_null($first_eastern_id);
      $result = mysql_query($query);
      $data = mysql_fetch_array($result, MYSQL_BOTH);
      ...
      // Collect data on second Eastern
      $query = "SELECT eastern.*, heraldic_rank.heraldic_rank FROM $DBNAME_AUTH.eastern LEFT OUTER JOIN $DBNAME_WARRANT.heraldic_rank ON eastern.heraldic_rank_id = heraldic_rank.heraldic_rank_id WHERE eastern.eastern_id = " . value_or_null($second_eastern_id);
      $result = mysql_query($query);
      $data = mysql_fetch_array($result, MYSQL_BOTH);
...
<p align="center" class="title2">Merge Easterner</p>
<p align="center" class="title2">Easterner Information</p>
<?php 
   if (isset($no_edit_selection) && $no_edit_selection)
   {
?>
<p align="center" class="title3" style="color:red">Two Easterners were not selected for Merge.  Please use a navigation link to the left.</p>
<?php 
   }
   else
   {
...
<?php 
      ...
      if (isset($first_eastern_id) && $first_eastern_id > 0)
      {
?>
   <input type="hidden" name="first_eastern_id" id="first_eastern_id" value="<?php echo $first_eastern_id; ?>"/>
<?php 
      }
      if (isset($second_eastern_id) && $second_eastern_id > 0)
      {
?>
   <input type="hidden" name="second_eastern_id" id="second_eastern_id" value="<?php echo $second_eastern_id; ?>"/>
<?php 
      }
?>
<table align="center" cellpadding="5" cellspacing="0" border="1">
   ...
   <tr>
      <td class="titleright" bgcolor="#FFFFCC">Easterner ID</td>
      <td class="data"><input type="radio" name="select_eastern_id" id="select_eastern_id" onclick="javascript:selectAll(1);" value="<?php echo $first_eastern_id; ?>" />Select All for Easterner ID <?php echo $first_eastern_id; ?></td>
      <td class="data"><input type="radio" name="select_eastern_id" id="select_eastern_id" onclick="javascript:selectAll(2);" value="<?php echo $second_eastern_id; ?>" />Select All for Easterner ID <?php echo $second_eastern_id; ?></td>
   </tr>
   <tr>
...
<?php
         ...
         for ($j = 0; $j < 2; $j++)
         {
            $a_id = $first_eastern_id;
            $s_name = $form_sca_name;
            if ($j == 1)
            {
               $a_id = $second_eastern_id;
            ...
            $award_query = "SELECT eastern_award.*, award.award_name, branch.branch, rg.branch AS rg, " .
                           ...
                           "FROM $DBNAME_OP.eastern_award JOIN $DBNAME_OP.award ON eastern_award.award_id = award.award_id " .
                           ...
                           "WHERE eastern_id = $a_id ORDER BY precedence, award_date, sequence";
...
?>
   <tr>
      <td>
         <input type="checkbox" name="form_eastern_award_id[]" id="form_eastern_award_id" value="<?php echo $eastern_award_id; ?>" checked="checked" />
      </td>
