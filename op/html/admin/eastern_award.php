...
$SUBMIT_SAVE = "Save Eastern Award";
$SUBMIT_DELETE = "Delete Eastern Award";
...
if (isset($_SESSION[$OP_ADMIN]) && $_SESSION[$OP_ADMIN])
{
   ...
   // Selected someone?
   $form_eastern_id = 0;
   if (isset($_REQUEST['eastern_id']))
   {
      $form_eastern_id = clean($_REQUEST['eastern_id']);
   }
   else if (isset($_POST['form_eastern_id']))
   {
   ...
   $form_eastern_award_id = 0;
   if (isset($_REQUEST['eastern_award_id']))
   {
      $form_eastern_award_id = clean($_REQUEST['eastern_award_id']);
   }
   else if (isset($_POST['form_eastern_award_id']))
   {
      $form_eastern_award_id = clean($_POST['form_eastern_award_id']);
   }
   ...
   if ($submit != "")
   {
      ...
      {
         $errmsg .= "Atlantia does not bestow grant-level Court Baronage - please select Court Baronage (AoA) for Eastern awards.<br/>";
      }
      ...
      if ($errmsg == "")
      {
         ...
         {
            $delete_query = "DELETE FROM $DBNAME_OP.eastern_award WHERE eastern_award_id = " . value_or_null($form_eastern_award_id);
         ...
         else if ($submit == $SUBMIT_SAVE)
         {
            ...
            // Update
            if ($mode == $MODE_EDIT && $form_eastern_award_id > 0)
            {
               ...
               $update_query = "UPDATE $DBNAME_OP.eastern_award SET " .
                               "eastern_id = " . value_or_null($form_eastern_id) .
                               ...
                               " WHERE eastern_award_id = " . value_or_null($form_eastern_award_id);

               $update_result = mysql_query($update_query) 
                  or die("UPDATE failed : " . mysql_error());
            }
            // Insert
            else // if ($mode == $MODE_ADD && $form_eastern_award_id == 0)
            {
               $insert_query = "INSERT INTO $DBNAME_OP.eastern_award (eastern_id, award_id, branch_id, award_date, sequence, premier, comments, retired_date, resigned_date, revoked_date, gender, event_id, court_report_id, last_updated, last_updated_by, private, scroll_status_id, scroll_assignees, scroll_assigned_date) VALUES (" .
                               value_or_null($form_eastern_id) . ", ".
               ...
               $form_eastern_award_id = mysql_insert_id();
               ...
               if ($form_award_id == $ST_AIDAN)
               {
                  ...
                  $insert_query = "INSERT INTO $DBNAME_OP.eastern_award (eastern_id, award_id, branch_id, award_date, sequence, comments, event_id, court_report_id, last_updated, last_updated_by, private, scroll_status_id, scroll_assignees, scroll_assigned_date) VALUES (" .
                                  value_or_null($form_eastern_id) . ", ".
                                  $AUG . ", " .
                                  $ATLANTIA . ", " .
                                  value_or_null($form_award_date) . ", " .
               ...
               else
               {
                  ...
                  {
                     $aidan_query = "SELECT eastern_award_id, award_id, award_date, retired_date FROM $DBNAME_OP.eastern_award WHERE eastern_id = " . value_or_null($form_eastern_id) . " AND award_id = " . $ST_AIDAN;
                     ...
                     if ($num > 0)
                     {
                        ...
                        $aidan_id = $aidan_data['eastern_award_id'];
                        if ($aidan_ret_date == "")
                        {
                           $aidan_update = "UPDATE $DBNAME_OP.eastern_award SET retired_date = " . value_or_null($form_award_date) . 
                                           ", last_updated = " . value_or_null(date("Y-m-d")) .
                                           ", last_updated_by = " . value_or_null($_SESSION["s_user_id"]) .
                                           " WHERE eastern_award_id = " . value_or_null($aidan_id);
                  ...
                  // Eastern Territorial Baronage GoA+ check
                  if ($type_id == $LANDED_BARONAGE || $type_id == $RETIRED_BARONAGE)
                  {
                     $arms_kingdom = get_kingdom_id($form_branch_id);
                     if ($arms_kingdom == $ATLANTIA)
                     {
                        $award_check_query = "SELECT eastern_award_id FROM $DBNAME_OP.eastern_award JOIN $DBNAME_OP.award ON eastern_award.award_id = award.award_id JOIN $DBNAME_OP.precedence ON precedence.type_id = award.type_id " .
                                             "WHERE precedence.precedence <= " . value_or_null($GOA_P) . " AND award_date < " . value_or_null($form_award_date) . " AND eastern_id = " . value_or_null($form_eastern_id);
                        ...
                        // Need to add GoA for Eastern Territorial Baronage not already of AoA rank
                        if ($num == 0)
                        {
                           $add_arms_query = "INSERT INTO $DBNAME_OP.eastern_award (eastern_id, award_id, branch_id, award_date, sequence, event_id, court_report_id, last_updated, last_updated_by, private, scroll_status_id, scroll_assignees, scroll_assigned_date) VALUES (" .
                                             value_or_null($form_eastern_id) . ", " .
                                             $GOA . ", " .
                                             $ATLANTIA . ", " .
                                             value_or_null($form_award_date) . ", " .
                  ...
                  {
                     $award_check_query = "SELECT eastern_award_id FROM $DBNAME_OP.eastern_award WHERE eastern_id = " . value_or_null($form_eastern_id) . " AND award_id = " . $POA;
                  }
                  // GOA Check
                  else if ($type_id == $ORDER_HIGH_MERIT || $type_id == $LANDED_BARONAGE || $type_id == $RETIRED_BARONAGE || $type_id == $COURT_BARONAGE_GOA)
                  {
                     $award_check_query = "SELECT eastern_award_id FROM $DBNAME_OP.eastern_award WHERE eastern_id = " . value_or_null($form_eastern_id) . " AND award_id = " . $GOA;
                  }
                  // AOA Check
                  else if ($type_id == $ORDER_MERIT || $type_id == $COURT_BARONAGE_AOA)
                  {
                     $award_check_query = "SELECT eastern_award_id FROM $DBNAME_OP.eastern_award WHERE eastern_id = " . value_or_null($form_eastern_id) . " AND award_id = " . $AOA;
                  }
                  ...
                  if (isset($award_check_query) && $award_check_query != '')
                  {
                     ...
                     if ($num == 0)
                     {
                        ...
                        if ($add_arms != "")
                        {
                           $add_arms_query = "INSERT INTO $DBNAME_OP.eastern_award (eastern_id, award_id, branch_id, award_date, sequence, last_updated, last_updated_by) VALUES (" .
                                             value_or_null($form_eastern_id) . ", " .
                                             $add_arms . ", " .
                                             $arms_kingdom . ", " .
                                             value_or_null($form_award_date) . ", " .
   ...
   {
      $eastern_query = 
         "SELECT eastern_id, sca_name ".
         "FROM $DBNAME_AUTH.eastern ".
         "WHERE eastern_id = ". $form_eastern_id ;

      $eastern_result = mysql_query($eastern_query) 
         or die("Eastern Query failed : " . mysql_error());

      $eastern_data = mysql_fetch_array($eastern_result, MYSQL_BOTH);

      $form_eastern_id = $eastern_data['eastern_id'];
      $form_scaname = trim($eastern_data['sca_name']);
   }

   if ($form_eastern_award_id > 0 && $submit != $SUBMIT_DELETE)
   {
      $idquery = 
         "SELECT eastern_award_id, award_id, award_date, sequence, premier, comments, retired_date, resigned_date, revoked_date, gender, event_id, court_report_id, private, " .
         "eastern_award.scroll_status_id, scroll_assignees, scroll_assigned_date, eastern_award.branch_id, branch, scroll_status, scroll_status_code " .
         "FROM $DBNAME_OP.eastern_award LEFT OUTER JOIN $DBNAME_BRANCH.branch ON eastern_award.branch_id = branch.branch_id " .
         "LEFT OUTER JOIN $DBNAME_OP.scroll_status ON eastern_award.scroll_status_id = scroll_status.scroll_status_id " .
         "WHERE eastern_award_id = ". value_or_null($form_eastern_award_id);
      ...
      $form_eastern_award_id = $iddata['eastern_award_id'];
   ...
   $title = ucfirst($mode) . " Eastern Award";
   include('header.php');
?>
<p class="title2"><?php echo ucfirst($mode); ?> Eastern Award</p>
<?php
...
<?php 
   ...
   if ($form_eastern_id > 0 && $form_award_id == 0 && $form_eastern_award_id == 0)
   {
      $award_query = NULL;
      $id_field = NULL;
      if ($mode == $MODE_EDIT)
      {
         $id_field = 'eastern_award_id';
         $award_query = 
            "(SELECT award.award_id, award.award_name, award.award_name_male, award.award_name_female, award.type_id, award.select_branch, " .
            "eastern_award.eastern_award_id, eastern_award.award_date, eastern_award.sequence, eastern_award.premier, eastern_award.retired_date, " .
            "branch.branch, branch.branch_id, precedence.precedence ".
            "FROM $DBNAME_OP.award, $DBNAME_OP.eastern_award, $DBNAME_BRANCH.branch, $DBNAME_OP.precedence ".
            "WHERE award.award_id = eastern_award.award_id  ".
            "AND award.type_id = precedence.type_id ".
            "AND award.branch_id = branch.branch_id ".
            "AND eastern_award.eastern_id = ". $form_eastern_id . ") ".
            "UNION ".
            "(SELECT award.award_id, award.award_name, award.award_name_male, award.award_name_female, award.type_id, award.select_branch, " .
            "eastern_award.eastern_award_id, eastern_award.award_date, eastern_award.sequence, eastern_award.premier, eastern_award.retired_date, " .
            "branch.branch, branch.branch_id, precedence.precedence ".
            ...
            "WHERE award.award_id = eastern_award.award_id  ".
            "AND award.type_id = precedence.type_id ".
            "AND eastern_award.branch_id = branch.branch_id ".
            "AND eastern_award.eastern_id = ". $form_eastern_id . ") ".
            "UNION ".
            "(SELECT award.award_id, award.award_name, award.award_name_male, award.award_name_female, award.type_id, award.select_branch, " .
            "eastern_award.eastern_award_id, eastern_award.award_date, eastern_award.sequence, eastern_award.premier, eastern_award.retired_date, " .
            "null AS branch, null AS branch_id, precedence.precedence ".
            "FROM $DBNAME_OP.award, $DBNAME_OP.eastern_award, $DBNAME_OP.precedence ".
            "WHERE award.award_id = eastern_award.award_id  ".
            "AND award.type_id = precedence.type_id ".
            "AND eastern_award.branch_id IS NULL ".
      ...
      $award_result = mysql_query($award_query) 
         or die("Award query failed : " . mysql_error());
   ?>
   <form action="eastern_award.php?mode=<?php echo $mode; ?>" method="post">
   <?php 
      if ($form_eastern_id > 0) 
      { 
   ?>
   ...
   ?>
   <tr>
      <th class="titleright">Eastern:</th>
   ...
   /*  PAGE 2 - Add/Edit Eastern Award */ 
   else if ($form_eastern_id > 0 && ($form_award_id > 0 || $form_eastern_award_id > 0) && (($submit != $SUBMIT_SAVE && $submit != $SUBMIT_DELETE) || $errmsg != ""))
   {
   ?>
   <form action="eastern_award.php?mode=<?php echo $mode; ?>" method="post">
   ...
   <?php 
      ...
      { 
   ?>
      <input name="form_eastern_id" id="form_eastern_id" type="hidden" value="<?php echo $form_eastern_id; ?>"/>
   <?php 
      }
      if ($form_award_id > 0) 
      { 
   ...
   <?php 
      ...
      { 
   ?>
      <input name="form_eastern_award_id" id="form_eastern_award_id" type="hidden" value="<?php echo $form_eastern_award_id; ?>"/>
   <?php 
      }
      if ($form_event_id > 0) 
      { 
...
?>
   <tr>
      <th class="titleright" valign="top">Eastern:</th>
      <td class="data"><?php echo $form_scaname; ?></td>
   </tr>
   <tr>
   ...
   <tr>
      ...
      <table border="0" align="center" cellpadding="5" summary="layout">
         ...
         <tr>
            ...
            <form action="eastern.php" method="post">
               <input type="hidden" name="form_eastern_id" id="form_eastern_id" value="<?php echo $form_eastern_id; ?>"/>
               <input type="hidden" name="mode" id="mode" value="<?php echo $MODE_EDIT; ?>"/>
               <input type="submit" value="Edit Eastern <?php echo $form_scaname; ?>"/>
            </form>
            </td>
         </tr>
...
include("header.php");
?>
   <p class="title2">Add/Edit Eastern Award</p>
