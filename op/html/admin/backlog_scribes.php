...
if (isset($_SESSION[$BACKLOG_ADMIN]) && $_SESSION[$BACKLOG_ADMIN])
{
   ...
   // Selected someone?
   $form_scribe = "";
   if (isset($_REQUEST['scribe']))
   {
      $form_scribe = clean($_REQUEST['scribe']);
   }
   else if (isset($_REQUEST['form_scribe']))
   {
      $form_scribe = clean($_REQUEST['form_scribe']);
   }
   $wc_scribe = "";
   if ($form_scribe != "")
   {
      $wc_scribe = " AND atlantian_award.scroll_assignees LIKE '%" . mysql_real_escape_string($form_scribe) . "%' ";
   }
   ...
   {
      $form_notes = clean($_REQUEST['notes']);
   }
   else if (isset($_REQUEST['form_notes']))
   {
      $form_notes = clean($_REQUEST['form_notes']);
   }
   $wc_notes = "";
   if ($form_notes != "")
   {
      $wc_notes = " AND atlantian_award.scroll_notes LIKE '%" . mysql_real_escape_string($form_notes) . "%' ";
   }
...
$query = "SELECT atlantian.sca_name, award.award_name, atlantian_award.award_date, atlantian_award.atlantian_award_id, " .
         ...
         "WHERE precedence.precedence <= $UNDER_OP_LEVEL " . $wc_scribe . $wc_notes .
...
<p align="center">Scrolls with Assigned Scribes '<b><?php echo $form_scribe; ?></b>' and Scroll Notes '<b><?php echo $form_notes; ?></b>'</p>
<?php
...
{
?>
<form action="backlog_scribes.php" method="post">
<input type="hidden" name="form_scribe" id="form_scribe" value="<?php echo $form_scribe; ?>" />
<input type="hidden" name="form_notes" id="form_notes" value="<?php echo $form_notes; ?>" />
<?php
}
?>
