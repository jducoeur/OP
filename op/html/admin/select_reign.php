...
{
   $form_reign_id = $_GET['reign_id'];
}
else if (isset($_POST['form_reign_id']))
{
   $form_reign_id = $_POST['form_reign_id'];
}
...
if ($submit == $SUBMIT_SELECT)
{
   if (isset($_POST['form_atlantian_id']))
   {
      ...
      if (!headers_sent($filename, $linenum)) 
      {
         if ($gender == $MALE)
         {
            redirect("reign.php?mode=$MODE_EDIT&form_reign_id=$form_reign_id&form_king_id=$form_atlantian_id");
         }
         else if ($gender == $FEMALE)
         {
            redirect("reign.php?mode=$MODE_EDIT&form_reign_id=$form_reign_id&form_queen_id=$form_atlantian_id");
         }
      }
      // Debugging
      else 
      {
         ...
         if ($gender == $MALE)
         {
            echo '<a href="reign.php?mode=' . $MODE_EDIT . '&form_reign_id=' . $form_reign_id . '&form_king_id=' . $form_atlantian_id . '">Continue</a>';
         }
         else if ($gender == $FEMALE)
         {
            echo '<a href="reign.php?mode=' . $MODE_EDIT . '&form_reign_id=' . $form_reign_id . '&form_queen_id=' . $form_atlantian_id . '">Continue</a>';
         }
         echo "<br/><br/>";
   ...
   {
      $errmsg2 = "Please select an Atlantian as Territorial King/Queen.";
      // Rerun the search
...
?>
<form action="select_reign.php" method="post">
...
<input type="hidden" name="form_reign_id" id="form_reign_id"<?php if (isset($form_reign_id) && $form_reign_id != 0) { echo " value=\"$form_reign_id\"";} ?>/>
