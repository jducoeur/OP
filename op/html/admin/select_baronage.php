...
{
   $form_baronage_id = $_GET['baronage_id'];
}
else if (isset($_POST['form_baronage_id']))
{
   $form_baronage_id = $_POST['form_baronage_id'];
}
...
if ($submit == $SUBMIT_SELECT)
{
   if (isset($_POST['form_atlantian_id']))
   {
      ...
      if (!headers_sent($filename, $linenum)) 
      {
         if ($gender == $MALE)
         {
            redirect("baronage.php?mode=$MODE_EDIT&form_baronage_id=$form_baronage_id&form_baron_id=$form_atlantian_id");
         }
         else if ($gender == $FEMALE)
         {
            redirect("baronage.php?mode=$MODE_EDIT&form_baronage_id=$form_baronage_id&form_baroness_id=$form_atlantian_id");
         }
      }
      // Debugging
      else 
      {
         ...
         if ($gender == $MALE)
         {
            echo '<a href="baronage.php?mode=' . $MODE_EDIT . '&form_baronage_id=' . $form_baronage_id . '&form_baron_id=' . $form_atlantian_id . '">Continue</a>';
         }
         else if ($gender == $FEMALE)
         {
            echo '<a href="baronage.php?mode=' . $MODE_EDIT . '&form_baronage_id=' . $form_baronage_id . '&form_baroness_id=' . $form_atlantian_id . '">Continue</a>';
         }
         echo "<br/><br/>";
   ...
   {
      $errmsg2 = "Please select an Atlantian as Baronage.";
      // Rerun the search
...
?>
<form action="select_baronage.php" method="post">
...
<input type="hidden" name="form_baronage_id" id="form_baronage_id"<?php if (isset($form_baronage_id) && $form_baronage_id != 0) { echo " value=\"$form_baronage_id\"";} ?>/>
